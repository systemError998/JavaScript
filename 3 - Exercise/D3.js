...
/* --EXTRA-- ESERCIZIO 9
  Utilizzando gli elementi presenti nell'array "femaleCharacters" rimuovi dall'array "characters" le stringhe corrispondenti a 
...
for(i=0; i<femaleCharacters.length; i++) {
  for(j=0; j<characters.lenght; j++) {
    if(femaleCharacters.name[i] === characters.name[j]) {
      sameName = characters.splice(j, 1) //All'indice j che è uguale a i elimino 1 ELEMENTO
    }
  }
} characters.splice(j, 1) //All'indice j che è uguale a i elimino 1 ELEMENTO
console.log(characters);
...
let random = Math.floor(Math.random() * starWarsCharacters.length)                // genera un indice casuale
let randomCharacters = starWarsCharacters[random];    // Ottieni l'elemento corrispondente all'indice casuale
